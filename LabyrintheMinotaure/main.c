...
void renameCharacter(char nom_hero[], char nom_princesse[]);//code utilisé pour changer les noms de la princesse et du héros
    ...
    while(clauseDeSortie)
    {
        if(kbhit())
        {
            ...
            if(touche == 72)    /// code ASCII de la touche flèche en haut
            {
                ...
                {
                    EffaceEcran();
                    printf("YOU WIN !\nYOUR SCORE: %d\nAppuyez sur Enter pour revenir au menu principal", score);
                    getc(stdin);
                    clauseDeSortie=0;
                }
            }
